 ...
 * z_bytes is the pre-hashed message digest.
 */
static bool key_ec_verify(struct l_key *key,
				const uint8_t *sig, size_t sig_len,
				const uint8_t *z_bytes, size_t z_len)
{
	return ecdsa_verify(key->ec_pubkey, z_bytes, z_len, sig, sig_len);
}

#define EC_DER_SIG_MAX  ECDSA_MAX_DER_SIG

/*
 * Software ECDSA signing.  Produces a DER Ecdsa-Sig-Value.
 * hash is the pre-hashed message digest (z).
 */
static ssize_t key_ec_sign(struct l_key *key,
				const uint8_t *hash, size_t hash_len,
				uint8_t *out, size_t out_len)
{
	return ecdsa_sign(key->ec_privkey, hash, hash_len, out, out_len);
}
