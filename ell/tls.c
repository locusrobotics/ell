					...
					struct l_key *priv_key)
{
	...
	if (priv_key) {
		...
		enum l_key_cipher_type cipher =
			l_key_get_type(priv_key) == L_KEY_ECC ?
			L_KEY_ECDSA_X962 : L_KEY_RSA_PKCS1_V1_5;

		tls->priv_key = priv_key;

		if (!l_key_get_info(tls->priv_key, cipher, L_CHECKSUM_NONE,
					&tls->priv_key_size, &is_public) ||
				is_public) {
